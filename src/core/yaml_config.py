...
class YAMLConfig(BaseConfig):
    def __init__(self, cfg_path: str, **kwargs) -> None:
        ...
        self.train_debug = cfg.get('train_debug', False)
        self.train_debug_steps = cfg.get('train_debug_steps', 3)


        print(f"[YAMLConfig] epoches={self.epoches}, log_step={self.log_step}, "
              f"use_amp={self.use_amp}, use_ema={self.use_ema}, "
              f"train_debug={self.train_debug}, train_debug_steps={self.train_debug_steps}")
    ...
    def model(self, ) -> torch.nn.Module:
        if self._model is None and 'model' in self.yaml_cfg:
            ...
            print(f"[YAMLConfig] model = {type(self._model).__name__}")
        return self._model
    ...
    def postprocessor(self, ) -> torch.nn.Module:
        if self._postprocessor is None and 'postprocessor' in self.yaml_cfg:
            ...
            print(f"[YAMLConfig] postprocessor = {type(self._postprocessor).__name__}")
    ...
    def criterion(self, ):
        if self._criterion is None and 'criterion' in self.yaml_cfg:
            ...
            print(f"[YAMLConfig] criterion = {type(self._criterion).__name__}")
    ...
    def optimizer(self, ):
        if self._optimizer is None and 'optimizer' in self.yaml_cfg:
            ...
            print(f"[YAMLConfig] optimizer = {type(self._optimizer).__name__}")
    ...
    def lr_scheduler(self, ):
        if self._lr_scheduler is None and 'lr_scheduler' in self.yaml_cfg:
            ...
            print(f"[YAMLConfig] lr_scheduler = {type(self._lr_scheduler).__name__}")
    ...
    def train_dataloader(self, ):
        if self._train_dataloader is None and 'train_dataloader' in self.yaml_cfg:
            ...
            print(f"[YAMLConfig] train_dataloader = {self._train_dataloader}")
    ...
    def val_dataloader(self, ):
        if self._val_dataloader is None and 'val_dataloader' in self.yaml_cfg:
            ...
            print(f"[YAMLConfig] val_dataloader = {self._val_dataloader}")
    ...
    def ema(self, ):
        if self._ema is None and self.yaml_cfg.get('use_ema', False):
            ...
            print(f"[YAMLConfig] ema = {type(self._ema).__name__}")
        return self._ema
    ...
    def scaler(self, ):
        if self._scaler is None and self.yaml_cfg.get('use_amp', False):
            ...
            print(f"[YAMLConfig] scaler = {type(self._scaler).__name__}")
        return self._scaler
